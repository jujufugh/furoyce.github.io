...
display_usage() {
    echo "Usage: $0 <compartment_id> <instance_name> <bucket_name> <src_dir>"
}
...
bucket_sync_downstream() {
    oci os object sync -bn "$bucket_name" --prefix "$bucket_folder_prefix" --dest-dir "$dest_dir"
}
...
# Sync source directory files to bucket
bucket_sync_upstream
...
echo "Bucket Folder Prefix: $bucket_folder_prefix"
echo "Source Directory: $src_dir"

echo "Source directory files synced to bucket"
